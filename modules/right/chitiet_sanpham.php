<?php
    ...
    $chitiet = sqlsrv_query($conn, $sql);
    $row_chitiet = sqlsrv_fetch_array($chitiet);
?>
<?php
echo
...
if (isset($_POST['binhluan'])) {
    ...
    if ($name_length > 0 && $comment_length > 0) {
        $sql = "insert into comment (id_sanpham,name,comment) values(?,?,?)";
        $data = [
            0 => $id,
            1 => $name,
            2 => $comment
        ];
        sqlsrv_query($conn, $sql, $data);
    } else {
    ...
    <table width="200" border="0" style="margin-bottom:40px;">
        ...
        <tr>
            ...
            <td width="122"><label for="ten"></label>
                <input type="text" name="ten" id="ten" size="30" placeholder="Tên bạn..."/>
            </td>
        </tr>
        <tr>
            <td>Nội dung</td>
...
<?php
    ...
    $ketqua_comment = sqlsrv_query($conn, $fine_comment);
    while ($dong_comment = sqlsrv_fetch_array($ketqua_comment)) {
