<?php
	...
	$row=sqlsrv_fetch_array($sanpham);
?>
  ...
  <tr>
    ...
    <td>
      ...
      <?php
			...
			$cat=sqlsrv_query($conn,$sql);
			while($dong_cat=sqlsrv_fetch_array($cat)){
  ...
  <tr>
    ...
    <td>  <select name="product_brand" id="product_brand">
      <?php
			...
			$hieu=sqlsrv_query($conn,$sql);
			while($dong=sqlsrv_fetch_array($hieu)){
